...
Teoria musical é uma API para ajudar na formação de escalas, acordes e campos harmônicos.

Toda a aplicação é baseada em uma aplicação chamada `teorial-musical`. Esse módulo tem um api's relacionadas a cada ação que a aplicação pode realizar. Como `escalas`, `acordes` e `campo-harmonico`
