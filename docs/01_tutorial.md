...
Se você chegou até aqui, significa que quer aprender mais sobre o `teoria-musical`.

O objetivo desse projeto é estudar música e obter fácil acesso a escalas, formação de acordes e campos harmônicos. Onde cada um desses comandos é distribuído em um subcomando da aplicação.
...
O `teoria-musical` distribui cada função em um subcomando e você executar cada um deles para testar agora. O objetivo desse tutorial é explicar o básico do funcionamento da aplicação em linha de comando.
